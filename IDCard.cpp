...
Mat IDCard::rotateImage1(Mat img, int degree, CvPoint2D32f center)
{
...
void IDCard::getIDNumberImg(const char* pSrcFileName, const char* pDstFileName)
{

	Mat srcImg = imread(pSrcFileName);
	Mat gray, binImg, adaImg;
	...
	//adaptiveThreshold(gray, adaImg, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 25, 10);
	//imwrite("C:\\Users\\sun\\Desktop\\ada.jpg", adaImg);
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		//	angle = 0;
		Mat rotate_img = rotateImage1(binImg, angle, rect.center);
...
int IDCard::splitNumbers(string file)
{
	Mat gray, bin;
	Mat srcImage = imread(file);

	cvtColor(srcImage, gray, COLOR_RGB2GRAY);
	threshold(gray, bin, 100, 255, CV_THRESH_BINARY);
	imwrite("C:\\Users\\sun\\Desktop\\rc1.jpg", srcImage); //将矫正后的图片保存下来

	vector<vector<Point> > contours;
	vector<Rect> boundRect(contours.size());
	//注意第5个参数为CV_RETR_EXTERNAL，只检索外框  
	findContours(bin, contours, CV_RETR_LIST, CV_CHAIN_APPROX_NONE); //找轮廓
	cout << contours.size() << endl;
	//drawContours(srcImage, contours, -1, Scalar(0, 0, 255), 2, 8);

	//imwrite("C:\\Users\\sun\\Desktop\\rc.jpg", srcImage); //将矫正后的图片保存下来
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		if (line1 >= line2 && line1 / line2 > 4)
		{
			continue;
		}
		if (line2 >= line1 && line2 / line1 > 4)
		{
			continue;
		}
		drawContours(idNumberImg, contours, i, Scalar(0, 0, 255), 2, 8);
		imwrite("C:\\Users\\sun\\Desktop\\rc.jpg", idNumberImg); //将矫正后的图片保存下来

		int xmax = 0, xmin = 0, ymax = 0, ymin = 0;

		xmax = rect.center.x + line2 / 2;
		xmin = rect.center.x - line2 / 2;
		ymax = rect.center.y + line1 / 2;
		ymin = rect.center.y - line1 / 2;


		//将身份证号区域截取出来
		Mat n(idNumberImg(Range(ymin, ymax), Range(xmin, xmax)));
		imwrite( "C:\\Users\\sun\\Desktop\\n.jpg", n);
		idNumbers.push_back(n);
